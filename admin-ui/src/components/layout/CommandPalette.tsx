...
export function CommandPalette() {
  ...
  const items = NAV.flatMap((g) =>
    g.items
      .filter((i) => {
        if (i.tenantOnly) return builtinTenantable && isAdmin();
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" onClick={() => setOpen(false)}>
      ...
      >
        ...
        <Command.List className="max-h-72 overflow-y-auto p-1">
          ...
          <Command.Group heading={t("command.navigation")} className="text-xs text-muted-foreground [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-1.5">
            {items.map((item) => (
              <Command.Item
                key={item.to}
                value={item.label}
          ...
          <Command.Group heading={t("command.actions")} className="text-xs text-muted-foreground [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-1.5">
            <Command.Item
              value="theme"
              onSelect={() => {
                toggleTheme();
            ...
            >
              ...
              {getTheme() === "dark" ? t("command.switchToLight") : t("command.switchToDark")}
            </Command.Item>
          </Command.Group>
        </Command.List>
      </Command>
