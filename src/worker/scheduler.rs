...
pub async fn create_schedule(
    ...
    mcms_derive::crud_insert!(pool, "cron_schedules", [
        "id" => id,
        "label" => label,
        "job_type" => job_type,
        "payload" => payload,
...
pub async fn toggle_schedule(pool: &Pool, id: SnowflakeId, enabled: bool) -> AppResult<()> {
    let now = crate::utils::tz::now_utc();
    let result: crate::db::pool::DbQueryResult = mcms_derive::crud_update!(pool, "cron_schedules",
...
pub async fn update_schedule(
    ...
    mcms_derive::crud_update!(pool, "cron_schedules",
        bind: [
...
pub async fn delete_schedule(pool: &Pool, id: SnowflakeId) -> AppResult<()> {
    let result: crate::db::DbQueryResult =
        mcms_derive::crud_delete!(pool, "cron_schedules", where: ("id", id))?;
            ...
            match &dispatch_result {
                Ok(()) => {
                    if let Some(ref lid) = log_id {
                        mcms_derive::crud_update!(&mut *tx, "cron_execution_log",
...
pub async fn create_execution_log(
    ...
    mcms_derive::crud_insert!(pool, "cron_execution_log", [
        "id" => id,
...
pub async fn complete_execution_log(
    ...
    let now = crate::utils::tz::now_utc();
    mcms_derive::crud_update!(pool, "cron_execution_log",
