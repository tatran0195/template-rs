...
use mcms::content_type::schema::{ContentTypeSchema, FieldType};
...
fn schema_to_ts(ct: &ContentTypeSchema) -> String {
    ...
    if ct.has_revision_routes()
        && !ct
            .fields
            .iter()
            .any(|f| f.name == mcms::constants::COL_VERSION)
    {
    ...
    if ct.implements_protocol("lockable")
        && !ct
            .fields
            .iter()
            .any(|f| f.name == mcms::constants::COL_LOCK_VERSION)
    {
...
fn field_type_to_ts(field: &mcms::content_type::schema::FieldSchema) -> String {
