...
pub async fn create(
    ...
    mcms_derive::crud_insert!(pool, "sms_codes", [
        "id" => id,
        "phone" => phone,
        "code" => code,
        "purpose" => purpose,
    ...
    let result: Option<SmsCode> =
        mcms_derive::crud_find!(pool, "sms_codes", SmsCode, where: ("id", id))?;
    result.ok_or_else(|| {
...
pub async fn find_by_id(pool: &crate::db::Pool, id: SnowflakeId) -> AppResult<Option<SmsCode>> {
    Ok(mcms_derive::crud_find!(pool, "sms_codes", SmsCode, where: ("id", id))?)
}
...
pub async fn is_rate_limited(
    ...
    let cnt = mcms_derive::crud_count!(
        pool,
        "sms_codes",
...
pub async fn verify_code(
    ...
    if sms.code != input_code {
        mcms_derive::crud_update!(pool, "sms_codes",
            bind: [],
