...
pub async fn list_roles(pool: &crate::db::Pool) -> AppResult<Vec<Role>> {
    ...
    let roles = mcms_derive::crud_list!(pool, "roles", Role, order_by: "name")?;
    Ok(roles)
}

/// Find role by id
pub async fn find_role_by_id(pool: &crate::db::Pool, id: SnowflakeId) -> AppResult<Option<Role>> {
    let role = mcms_derive::crud_find!(pool, "roles", Role, where: ("id", id))?;
    Ok(role)
}
...
pub async fn delete_role(pool: &crate::db::Pool, id: SnowflakeId) -> AppResult<()> {
    mcms_derive::crud_delete!(pool, "roles", where: ("id", id))?;
    Ok(())
}
...
pub async fn find_permissions_by_role_id(
    ...
    let perms = mcms_derive::crud_find_all!(pool, "permissions", Permission, where: ("role_id", role_id), order_by: "action")?;
    Ok(perms)
}
...
pub async fn delete_permissions_by_role_id(
    ...
    mcms_derive::crud_delete!(pool, "permissions", where: ("role_id", role_id))?;
    Ok(())
}
...
pub async fn insert_permission(pool: &crate::db::Pool, cmd: &CreatePermissionCmd) -> AppResult<()> {
    ...
    mcms_derive::crud_insert!(pool, "permissions", [
        "id" => id,
        "role_id" => cmd.role_id,
        "action" => &cmd.action,
        "subject" => &cmd.subject,
