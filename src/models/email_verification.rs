...
pub async fn create(
    ...
    mcms_derive::crud_insert!(pool, "email_verification_tokens", [
        "id" => id,
        "user_id" => user_id,
        "token" => &token,
        "email" => email,
...
pub async fn mark_verified(pool: &crate::db::Pool, id: SnowflakeId) -> AppResult<()> {
    ...
    mcms_derive::crud_update!(pool, "email_verification_tokens",
...
pub async fn cleanup_expired(pool: &crate::db::Pool) -> AppResult<u64> {
    mcms_derive::check_schema!("email_verification_tokens", "expires_at", "verified_at");
    let now = crate::utils::tz::now_utc();
    let sql = format!(
...
pub async fn tx_mark_verified(
    ...
    mcms_derive::crud_update!(&mut *tx, "email_verification_tokens",
        bind: ["verified_at" => now],
        where: ("id", id)
    )?;
    Ok(())
